...
class AffectedVarMethodVisitor(
            ...
            when (insn) {
                is MethodInsnNode -> {
                    if (insn.opcode != Opcodes.INVOKESTATIC &&
                        insn.opcode != Opcodes.INVOKEDYNAMIC) {
                        val value =
                            try {
                                frame?.getStack(
                                    frame.stackSize - Type.getArgumentTypes(insn.desc).size - 1)
                        ...
                        for (src in value.insns) {
                            ...
                            if (src is FieldInsnNode &&
                                src.opcode == Opcodes.GETFIELD &&
                                !isStatic) {
                                val fieldValue =
                                    srcFrame?.getStack(srcFrame.stackSize - 1) ?: continue
                            ...
                            // If a method call is from a local object, check if the object is on
                            // the stack.
                            if (src is VarInsnNode && src.opcode == Opcodes.ALOAD) {
                                try {
                                    if (throwInsnFrame?.getLocal(src.`var`) ==
                                        srcFrame?.getLocal(src.`var`)) {
                        ...
                        Opcodes.PUTFIELD -> {
                            val objRef =
                                try {
                                    ...
                                    logger.error {
                                        "Processing instruction $insn failed. Error: $e"
                                    }
                                    continue
                                } ?: continue
            ...
            when (src) {
                is MethodInsnNode -> {
                    if (src.opcode != Opcodes.INVOKESTATIC && src.opcode != Opcodes.INVOKEDYNAMIC) {
                        val srcRef =
                            srcFrame.getStack(
                                srcFrame.stackSize - Type.getArgumentTypes(src.desc).size - 1)
                ...
                is FieldInsnNode -> {
                    when (src.opcode) {
                        Opcodes.GETFIELD -> {
                            ...
                            processSourceValue(
                                src, srcRef, frames, throwInsnFrame, localVariableMap)
                        }
                        Opcodes.GETSTATIC -> {
    ...
    fun findSourceInsn(
        ...
        try {
            ...
            if (isThrowInsn &&
                    ...
                    if (throwInsnLocal is FieldInsnNode) {
                        ...
                        if (throwInsnLocal.opcode != Opcodes.INVOKEDYNAMIC &&
                            ...
                            throwInsnFrame.getStack(
                                throwInsnFrame.stackSize -
                                    Type.getArgumentTypes(throwInsnLocal.desc).size -
                                    1)
                        } else {
                            null
                        }
                ...
                if (objRef != null) {
                    processSourceValue(
                        throwInsnLocal, objRef, frames, throwInsnFrame, localVariableMap)
                }
                return
            }
            ...
            if (currentInsn is JumpInsnNode) {
                ...
                for (objRef in objRefs) {
                    processSourceValue(
                        currentInsn, objRef, frames, throwInsnFrame, localVariableMap)
                }
            }
        } catch (e: IndexOutOfBoundsException) {
    ...
    override fun visitEnd() {
        ...
        if (throwInsn != null && (catchIndex == -1L || catchInsn != null)) {
            ...
            val throwInsnIndex = instructions.indexOf(throwInsn)
            val interpreter =
                AffectedVarInterpreter(instructions, throwInsnIndex, tryCatchLocations)
            ...
            if (catchIndex != -1L) {
                val normalRunInterpreter =
                    AffectedVarInterpreter(instructions, throwInsnIndex, emptyList())
