...
import static al.aoli.exchain.phosphor.instrumenter.splitter.Util.OBJECT_TYPE;
...
public class Splitter implements Iterable<Splitter.SplitPoint> {
    ...
    /**
     * @param api Same as for or any other ASM class
    ...
    public static class SplitPoint {
        /**
         * The locals read in this split area, keyed by index. Value type is always int, float,
         * long, double, or object.
         */
        public final SortedMap<Integer, Type> localsRead;
        /**
         * The locals written in this split area, keyed by index. Value type is always int, float,
         * long, double, or object.
         */
        public final SortedMap<Integer, Type> localsWritten;
        /**
         * The values of the stack needed at the start of this split area. Type is always int,
         * float, long, double, or object.
         */
        public final List<Type> neededFromStackAtStart;
        /**
         * The values of the stack at the end of this split area that are needed to put back on the
         * original. Type is always int, float, long, double, or object.
         */
        ...
        public SplitPoint(
                SortedMap<Integer, Type> localsRead,
                SortedMap<Integer, Type> localsWritten,
                List<Type> neededFromStackAtStart,
                List<Type> putOnStackAtEnd,
                int start,
                int length) {
    ...
    protected class Iter implements Iterator<SplitPoint> {
        ...
        protected Iter() {
            ...
            // Must be sorted by earliest starting index then earliest end index then earliest
            // handler
            tryCatchBlocks.sort(
                    (o1, o2) -> {
        ...
        protected SplitPoint longestForCurrIndex() {
            ...
            // Now that we have our largest range from the start index, we can go over each updating
            // the local refs and stack
            ...
            for (TryCatchBlockNode block : tryCatchBlocks) {
                ...
                if (info.startIndex <= startIndex && info.endIndex >= endIndex) {
                    ...
                    if (handleIndex < info.startIndex || handleIndex > info.endIndex) {
                        // Well, it's not within, so that means we can't include this try block at
                        // all
                    ...
                    // The end has to be changed to the block's end so it doesn't go over the
                    // boundary
                ...
                // If the catch is within and not allowed to be, we have to change the end to before
                // it
                if (catchWithinDisallowed
                        && info.startIndex <= handleIndex
                        && info.endIndex >= handleIndex) {
        ...
        protected void constrainEndByExternalJumps(InsnTraverseInfo info) {
            // Basically, if any external jumps jump into our range, that can't be included in the
            // range
            for (int i = 0; i < insns.length; i++) {
                ...
                if (node instanceof JumpInsnNode) {
                    ...
                    if (index >= info.startIndex)
                        info.endIndex = Math.min(info.endIndex, index - 1);
                } else if (node instanceof TableSwitchInsnNode) {
        ...
        protected SplitPoint splitPointFromInfo(InsnTraverseInfo info) {
            // We're going to use the analyzer adapter and run it for the up until the end, a step
            // at a time
            StackAndLocalTrackingAdapter adapter = new StackAndLocalTrackingAdapter(Splitter.this);
            // Visit all of the insns up our start.
            // XXX: I checked the source of AnalyzerAdapter to confirm I don't need any of the
            // surrounding stuff
            ...
            return new SplitPoint(
                    ...
                    typesFromAdapterStackRange(
                            stackAtStart, adapter.lowestStackSize, adapter.uninitializedTypes),
                    typesFromAdapterStackRange(
                            adapter.stack, adapter.lowestStackSize, adapter.uninitializedTypes),
                    info.startIndex,
                    info.getSize());
        }
        ...
        protected Type typeFromAdapterStackItem(
                Object item, Map<Object, Object> uninitializedTypes) {
            if (item == Opcodes.INTEGER) return Type.INT_TYPE;
                ...
                return Type.getObjectType((String) uninitializedTypes.get(item));
    ...
    protected static class StackAndLocalTrackingAdapter extends AnalyzerAdapter {
        ...
        protected StackAndLocalTrackingAdapter(Splitter splitter) {
            super(
                    splitter.api,
                    splitter.owner,
                    splitter.method.access,
                    splitter.method.name,
                    splitter.method.desc,
                    null);
            stack =
                    new SizeChangeNotifyList<Object>() {
                        @Override
