...
import static al.aoli.exchain.phosphor.instrumenter.splitter.Util.*;
...
public class SplitMethod {
    ...
    /**
     * Calls {@link #split(String, MethodNode, int, int, int)} with minSize as 20% + 1 of the
     * original, maxSize as 70% + 1 of the original, and firstAtLeast as maxSize. The original
     * method is never modified and the result can be null if no split points are found.
     */
    ...
    /**
     * Splits the given method into two. This uses a {@link Splitter} to consistently create {@link
     * #fromSplitPoint(String, MethodNode, Splitter.SplitPoint)}.
     *
     * @param owner The internal name of the owning class. Needed when splitting to call the split
     *     off method.
     ...
     * @param firstAtLeast The number of instructions that, when first reached, will immediately be
     *     used without continuing. Since split points are streamed, this allows splitting without
     *     waiting to find the largest overall. If this is &lt= 0, it will not apply and all split
     *     points will be checked to find the largest before doing the split.
     * @return The resulting split method or null if there were no split points found
     */
    public Result split(
            String owner, MethodNode method, int minSize, int maxSize, int firstAtLeast) {
    ...
    /**
     * Split the given method at the given split point. Called by {@link #split(String, MethodNode,
     * int, int, int)}. The original method is never modified.
     */
    ...
    protected MethodNode createSplitOffMethod(MethodNode orig, Splitter.SplitPoint splitPoint) {
        // The new method is a static synthetic method named method.name + "$split" that returns an
        // object array
        ...
        MethodNode newMethod =
                new MethodNode(
                        ...
                        Type.getMethodDescriptor(
                                Type.getType(Object[].class), args.toArray(new Type[0])),
                        null,
                        null);
        ...
        // Next set of instructions comes verbatim from the original, but we have to change the
        // local indexes
        ...
        // Now go over each stack item and load the arr, swap w/ the stack, add the index, swap with
        // the stack, and store
    ...
    protected MethodNode createTrimmedMethod(
            String owner, MethodNode orig, MethodNode splitOff, Splitter.SplitPoint splitPoint) {
        // The trimmed method is the same as the original, yet the split area is replaced with a
        // call to the split off
        // portion. Before calling the split-off, we have to add locals to the stack part. Then
        ...
        MethodNode newMethod =
                new MethodNode(
                        ...
                        orig.desc,
                        orig.signature,
                        orig.exceptions.toArray(new String[0]));
        ...
        splitPoint.localsRead.forEach(
                (index, type) -> {
                    ...
                    // TODO: safe? if not, maybe just put at the top of the method a bunch of
                    // defaulted locals?
                    if (seenStoredLocals.contains(index))
                        newMethod.visitVarInsn(loadOpFromType(type), index);
        ...
        for (Integer index : splitPoint.localsWritten.keySet()) {
            ...
            if (!item.equals(OBJECT_TYPE)) {
                newMethod.visitTypeInsn(
                        Opcodes.CHECKCAST, boxedTypeIfNecessary(item).getInternalName());
            }
            // Unbox if necessary
        ...
        for (int i = 0; i < splitPoint.putOnStackAtEnd.size(); i++) {
            ...
            if (!last) {
                // Note if the stack item takes two slots, we do a form of dup then pop since
                // there's no swap1x2
